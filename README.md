...
<img width="382" height="589" alt="Image" src="https://github.com/user-attachments/assets/b4fa5f11-1c7e-4699-85e9-c4f1dcda3f7f" />
<img width="394" height="589" alt="Image" src="https://github.com/user-attachments/assets/6c754dc5-6f31-4f78-b1fa-439b6dffe043" />
...
UAC 권한이 자동으로 요청됩니다.(`Set-NetIPAddress` 가 관리자 권한을 필요로 함)
...
> `preset.init_presets()` 가 앱 시작 즉시 자동으로 생성합니다. EXE 옆에 어떤 데이터
> 파일도 두지 않으며, 별도 설치 절차나 디렉터리 권한 설정도 필요하지 않습니다.



## 사용법
1. 네트워크 어댑터 선택(어댑터 변경사항 발생시 새로고침 필요)
2. IP 주소 및 DNS 수동 설정 또는 DHCP 설정
3. 프리셋에서 현재값 혹은 수동으로 설정값 지정 가능
4. 지정된 어댑터 프리셋 버튼으로 간단한 IP 변경
...
### 위젯(트레이) 모드 — 신규
- **X 버튼 → "트레이 / 완전 종료" 선택 다이얼로그**
- 트레이로 내려도 **프로세스가 계속 살아 있음**:
